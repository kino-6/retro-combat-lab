...
import { growthRank, moraleHitModifier, weaponDamageModifier, weaponHitModifier } from './characterRules.js';
...
export function meleeHitChance(state: GameState, heavy: boolean): number {
  ...
  return clamp((heavy ? 0.72 : 0.86) - state.combat.distance * 0.19 + weaponHitModifier(state) + guardBonus + moraleHitModifier(state) + growthRank(state, 'melee') * 0.025, 0.1, 0.96);
}
...
export function rangedHitChance(state: GameState): number {
  ...
  const backgroundBonus = state.backgroundId === 'mechanic' ? 0.07 : state.backgroundId === 'hunter' ? 0.05 : 0;
  return clamp(byDistance + intellectBonus + backgroundBonus + moraleHitModifier(state) + growthRank(state, 'firearms') * 0.04, 0.22, 0.97);
}
...
export function thrownHitChance(state: GameState): number {
  ...
  return clamp(byDistance + intellectBonus + moraleHitModifier(state) + fieldcraftBonus, 0.18, 0.88);
}
...
export function expectedDamage(state: GameState, action: CombatAction): number {
  ...
  if (action === 'shoot') {
    ...
    const backgroundBonus = state.backgroundId === 'mechanic' ? 2 : state.backgroundId === 'hunter' ? 2 : 0;
    return Math.max(3, state.player.attack + 1 + distanceBonus + backgroundBonus + growthRank(state, 'firearms') * 2);
  }
  if (action === 'shotgun') {
    const closeBonus = Math.max(0, 4 - state.combat.distance) * 2;
    const firearmBonus = growthRank(state, 'firearms') * 2;
    return Math.max(4, state.player.attack + 4 + closeBonus + firearmBonus);
  }
  if (action === 'grenade') {
    return 16 + growthRank(state, 'fieldcraft') * 2;
  }
